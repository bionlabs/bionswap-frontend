...
const InputTicketModal = ({
  ...
  const totalDeposit = balanceAirdropTicket + balanceNormalTicket;
  ...
  const deposit = async () => {
    try {
      if (!parentContract || !account || !bionTicketContract || shareOf > 0) return;
      if (isApprovedForAll) {
        setIsLoading(true);
        ...
        <Stack width="100%" gap="20px">
          ...
          <PrimaryLoadingButton onClick={deposit} isLoading={isLoading} sx={{height: '41.59px'}}>
            <Typography variant="body3Poppins" fontWeight="600" color="#000000">
              {isApprovedForAll ? 'Comfirm' : 'Approve'}
            </Typography>
